...
class FakePrivacyAppSettingsDataSource implements AppSettingsLocalDataSource {
...
class FakePrivacyVehicleDataSource implements VehicleLocalDataSource {
...
void main() {
  ...
  setUp(() {
    ...
    settingsRepo = AppSettingsRepository(localDataSource: fakeSettings);
    vehiclesRepo = VehiclesRepository(localDataSource: fakeVehicles);
    preferencesRepo =
        VehiclePreferencesRepository(localDataSource: fakePreferences);
  });

  Widget buildTestApp({Widget? home, RouteFactory? onGenerateRoute}) {
  ...
  group('Privacy Policy & Data Usage Consent Flow Tests', () {
    testWidgets(
        ...
        (tester) async {
      await tester.pumpWidget(buildTestApp(home: const AppStartupScreen()));
    ...
    testWidgets(
      ...
      expect(find.text('What We Don’t Collect'), findsOneWidget);
      expect(find.text('Device Data Notice'), findsOneWidget);
      expect(find.text('Read Full Privacy Policy'), findsNothing);
    ...
    testWidgets(
      ...
      expect(fakeSettings.getPrivacyPolicyAcceptedVersion(), isNull);
      expect(fakeSettings.isOnboardingCompleted(), isFalse);

      final acceptBtn = find.text('I Understand & Continue');
    ...
    testWidgets(
      ...
      expect(find.byType(PrivacyConsentScreen), findsOneWidget);
      expect(find.text('Updated Privacy Policy'), findsOneWidget);
      expect(find.text('Read Full Privacy Policy'), findsNothing);
    ...
    testWidgets(
        'Settings screen displays Privacy Policy tile with official external link configuration',
        (tester) async {
      await tester.pumpWidget(buildTestApp(home: const SettingsScreen()));
      await tester.pumpAndSettle();

      expect(find.byType(PrivacySettingTile), findsOneWidget);
      expect(find.text('Privacy Policy'), findsOneWidget);
      expect(find.text('Read how Odomex handles your data'), findsOneWidget);
      expect(AppConstants.privacyPolicyUrl, 'https://hexakode.in/odomex/privacy');
    });
